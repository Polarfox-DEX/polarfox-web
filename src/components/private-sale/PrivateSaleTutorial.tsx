  ...
  return (
    <div className={classNames('container', className)}>
      ...
      <div className="grid tablet:grid-cols-2 laptop:grid-cols-4 desktop:grid-cols-7 desktop:px-4">
        <PresaleTutorialBlock logo={<CheckCircle className="fill-current" />} title="1 - Get whitelisted">
...
function PresaleTutorialBlock({ logo, title, className, children }: PresaleTutorialBlockProps) {
  return (
    <div className={classNames('container mt-12', className)} style={{ lineHeight: calcRem(30) }}>
      {logo}
...
export function GreyRightArrow() {
  return (
    <RightArrow className="fill-current opacity-30 hidden desktop:block w-full" style={{ marginTop: calcRem(103) }} />
  )
}
